...
async function fetchArticle(id: string | number): Promise<Article | null> {
  try {
    ...
    const url = base ? `${base.replace(/\/$/, '')}/articles/${id}` : `/api/dev/articles/${id}`
              ...
              {tokens.map((tok, idx) => {
                ...
                return (
                  ...
                  >
                    {tok}
                    {def && (
                      <span className="ml-1 text-xs align-middle text-blue-500 hidden md:inline">▾</span>
                    )}
                  </span>
                )
              })}
          ...
          <aside className="space-y-4">
            <div className="sticky top-24 bg-white p-4 rounded-md shadow-sm">
              ...
              <div className="mt-4">
                ...
                <div className="text-sm text-gray-700">This is an interactive reading view for the article.</div>
              </div>
            </div>
            ...
            <div className="bg-white p-4 rounded-md shadow-sm">
              ...
              <ul className="text-sm space-y-2">
                {Array.from(new Set(tokens.filter((t) => /^\w+$/.test(t) && t.length > 5))).slice(0, 6).map((w, i) => {
